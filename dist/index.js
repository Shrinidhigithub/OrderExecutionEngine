...
const worker_2 = require("./queue/worker");
...
fastify.get('/health', async (request, reply) => {
    ...
    try {
        await (0, orderStore_1.dbHealthCheck)();
        result.database = 'ok';
    }
    catch (e) {
    ...
    try {
        const ok = await (0, pubsub_1.redisHealthCheck)();
...
fastify.post('/api/orders/execute', async (request, reply) => {
    ...
    const order = { id, tokenIn: body.tokenIn, tokenOut: body.tokenOut, amount: Number(body.amount), status: 'pending' };
    await (0, orderStore_1.createOrder)(order);
    await worker_1.orderQueue.add('execute', order, { attempts: 3, backoff: { type: 'exponential', delay: 500 } });
...
fastify.register(async function (fastify) {
    fastify.get('/api/orders/execute', { websocket: true }, (connection, req) => {
        ...
        if (orderId) {
            unsubscribe = (0, pubsub_1.subscribeOrderUpdates)(orderId, (msg) => {
                try {
        ...
        ws.on('message', async (raw) => {
            try {
                ...
                if (data && data.tokenIn && data.tokenOut && data.amount) {
                    const id = (0, uuid_1.v4)();
                    const order = { id, tokenIn: data.tokenIn, tokenOut: data.tokenOut, amount: Number(data.amount), status: 'pending' };
                    await (0, orderStore_1.createOrder)(order);
                    await worker_1.orderQueue.add('execute', order, { attempts: 3, backoff: { type: 'exponential', delay: 500 } });
                    ws.send(JSON.stringify({ orderId: id }));
                    const unsub = (0, pubsub_1.subscribeOrderUpdates)(id, (msg) => {
                        try {
                            ws.send(JSON.stringify(msg));
                        }
                        catch (e) {
...
const start = async () => {
    try {
        ...
        (0, pubsub_1.initRedis)().catch(e => console.warn('Redis init error (non-fatal):', e));
        // Start worker pool with error handling
        console.log('Starting worker pool...');
        try {
            (0, worker_2.startWorker)(10);
            console.log('✓ Worker pool started with 10 concurrent workers');
        }
        catch (e) {
            console.warn('⚠ Worker pool startup warning:', e?.message || e);
